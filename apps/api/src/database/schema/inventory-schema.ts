...
import { check, foreignKey, index, pgEnum, pgTable, unique } from "drizzle-orm/pg-core";
...
export const stockMoves = pgTable(
  ...
  (t) => ({
    ...
    productVariantId: t
      ...
      .notNull(),
    fromLocationId: t.uuid().references(() => locations.id, { onDelete: "restrict" }),
    ...
    notes: t.text(),
    purchaseOrderId: t.uuid().references(() => purchaseOrders.id, { onDelete: "restrict" }),
    requisitionId: t.uuid().references(() => requisitions.id, { onDelete: "restrict" }),
    createdAt: t.timestamp().defaultNow().notNull(),
  }),
  (t) => [
...
export const stockLevels = pgTable(
  ...
  (t) => [
    unique("stock_levels_productVariant_location_uidx").on(t.productVariantId, t.locationId),
