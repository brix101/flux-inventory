...
> extends Context.Service<Self, HttpApiClient.Client<Groups, never, never>> {
  ...
  >(
    ...
    ? (
        // Simplify<HttpApiEndpoint.ClientRequest<_Params, _Query, _Payload, _Headers, "decoded-only">>
        variables: _Payload["Type"],
      ) => Promise<ResponseByMode<_Success["Type"], ResponseMode>>
    : never;
...
export const Service =
  ...
  <const Id extends string, ApiId extends string, Groups extends HttpApiGroup.Any>(
    ...
    self.runtime = ManagedRuntime.make(
    ...
    self.mutation = ((group: string, endpoint: string) => (opts: any) => {
      // TODO: put the params, in opts in the future for other Methods like PUT, DELETE, etc. For now, we only support POST requests with a payload, so we can assume that all the variables are in the payload.
      return self.runtime.runPromise(
        ...
        self.use((client) =>
          catchErrors(client[group][endpoint]({ payload: opts }) as Effect.Effect<any>),
        ),
      );
    }) as any;
    ...
    self.query = ((
      ...
      return self.runtime.runPromise(
        ...
        self.use((client) =>
          catchErrors(
            client[group][endpoint](key) as Effect.Effect<
              any,
